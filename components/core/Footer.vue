<template>
  ...
  >
    <v-row align="center" justify="space-between">
      ...
      <v-col class="pb-3 text-center" cols="12" md="4" order="1" order-sm="0">
        &copy; {{ new Date().getFullYear() }} BoQuest Global. All rights
        reserved.
      </v-col>
      <v-col class="text-center text-md-right" cols="12" md="4">
        <v-btn
          ...
          small
          @click.stop="
            social.link ? redirect_to(social.link) : (wechatQRCodeOpened = true)
          "
        >
          <v-icon class="mr-1" v-text="social.icon" />
        </v-btn>
      </v-col>
    </v-row>
    <v-dialog v-model="wechatQRCodeOpened" max-width="200">
      <v-card>
        <v-img height="200" width="200" :src="wechat_qr_code"></v-img>
      </v-card>
    </v-dialog>
  </v-footer>
</template>
